...
### 2.3 当前实现兼容性与已知限制
- 支持：列表/字典/集合推导式（显式循环求值，无 eval）、生成器表达式（惰性，Python 生成器包装）、基础算术/比较/逻辑/字典解包，局部 import，sys.argv 在执行前统一初始化为至少含一个占位字符串的列表。
- f-string：支持单/双/三引号（含多行）、格式化说明符（如 `:0>6.2f`），表达式可引用当前已知变量/模块；复杂嵌套插值仍不建议。
- 未覆盖：async/await 等异步语法；None 参与的某些一元/关系运算仍需谨慎（将返回脚本错误而非抛异常）。
- 建议：使用显式空格缩进（避免 Tab/空格混用），避免依赖 Python 3 语法的尖端特性。
...
### 6.7 字符串与 f-string 支持
- 普通字符串：支持单引号/双引号/三引号（多行）。
- f-string：支持单/双/三引号（含多行），支持格式化说明符（例如 `f\"{3.14159:0>6.2f}\"`）；内部表达式可访问当前已知变量/模块。复杂嵌套插值不建议使用。
- 建议：保持字符串与缩进格式清晰，避免在 f-string 中使用过深的嵌套或异常字符混用。
...
3. **推导式支持**：函数体内支持列表/字典/集合推导式（显式循环求值，无 eval）；生成器表达式通过 Python 辅助生成器保持惰性
4. **自动错误抑制**：在函数定义阶段，语法树访问器会跳过函数体内部表达式的求值，避免解析错误
5. **f-string 支持**：支持单/双/三引号（含多行）f-string，支持格式化说明符，表达式可引用当前已知变量/模块

...
### 2.3 当前实现兼容性与已知限制
- 支持：列表推导式（函数体内），基础算术/比较/逻辑/字典解包，局部 import，sys.argv 在执行前被统一初始化为至少含一个占位字符串的列表。
- f-string：已支持单行 `f"..."` / `f'...'`，内部表达式可引用当前全局变量；暂不支持三引号、多行、格式化说明符。
- 推导式：列表推导式可用；字典/集合推导式未充分验证；生成器表达式、async/await 不支持。
- 未覆盖：复杂格式说明符、复杂嵌套 f-string，生成器表达式，异步语法。
...
- 普通字符串：支持单引号/双引号；目前不推荐使用三引号多行字符串。
- f-string：支持单行 `f"..."` / `f'...'`，内部表达式可访问当前全局变量；暂不支持三引号、多行或格式化说明符。
- 建议：若需要简单插值，可用 `f"hello {name}"`；更复杂的格式化可回退到 `"hello {}".format(name)`。
...
3. **推导式支持**：函数体内支持列表推导式；字典/集合推导式未全面验证，生成器表达式暂不支持
4. **自动错误抑制**：在函数定义阶段，语法树访问器会跳过函数体内部表达式的求值，避免解析错误
5. **f-string 支持**：支持单行 f-string，内部表达式可引用全局变量；暂不支持三引号、多行或格式说明符
